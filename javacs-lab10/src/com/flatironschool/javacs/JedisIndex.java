...
public class JedisIndex {
	...
	public void indexPage(String url, Elements paragraphs) {
        // FILL THIS IN!
				...
				//if indexed already, delete old Hash it
				if (isIndexed(url))
				{
					jedis.del(termCounterKey(url));
				}

				// make a Jedis TermCounter and count the terms in the paragraphs
				TermCounter tc = new TermCounter(url);
				tc.processElements(paragraphs);
				for (String term : tc.keySet())
				{
					Integer termCount = tc.get(term);
					jedis.hset(termCounterKey(url), term, termCount.toString());
